...
class VADGate(FrameProcessor):
    ...
    async def process_frame(self, frame: Frame, direction: FrameDirection):
        ...
        try:
            ...
            if isinstance(frame, AudioRawFrame):
                # Validate the number of samples before processing
                if not self.validate_audio_frame(frame):
                    logger.error(f"Invalid number of samples in audio frame: {len(frame.audio)}")
                    return
    ...
    def validate_audio_frame(self, frame: AudioRawFrame) -> bool:
        num_samples = len(frame.audio) // 2  # assuming 16-bit samples
        if frame.sample_rate == 16000 and num_samples != 512:
            return False
        if frame.sample_rate == 8000 and num_samples != 256:
            return False
        return True
