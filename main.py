...
def start_server():
    while True:
        process = subprocess.Popen(
            f"python -m vllm.entrypoints.openai.api_server --port 5000 --model unsloth/Meta-Llama-3-8B-bnb-4bit --api-key {openai_api_key}",
            shell=True
        )
...
async def main(room_url: str, token: str):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        llm = OpenAILLMService(
            ...
            model="unsloth/Meta-Llama-3-8B-bnb-4bit",
